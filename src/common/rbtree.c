...
rb_node _rb_successor(rb_node node) {
    if(!node) return NULL;
    rb_node right_child = node->rb_right;
    if(right_child){
        while(right_child->rb_left){
            right_child = right_child->rb_left;
        }
        return right_child;
    }
    rb_node parent = rb_parent(node);
    while(parent != NULL && node == parent->rb_right){
        node = parent;
...
rb_node _rb_precursor(rb_node node) {
    if (!node) return NULL;
    // 1.有左孩子 找左子树中最右边的值
    rb_node left_child = node->rb_left;
    if (left_child) {
        while (left_child->rb_right) {
            left_child = left_child->rb_right;
        }
        return left_child;
    }
    // 没有左孩子 则判断在其父节点的位子
    ...
    rb_node parent = rb_parent(node);
    while (parent != NULL && node == parent->rb_left) {
        node = parent;
        parent = rb_parent(parent);
    }
    return parent;
